"""Distributed circuit breaker for cross-process failure isolation.

Extracted from ``distributed.py`` to keep file sizes manageable.
...
class DistributedCircuitBreaker:
    ...
    def state(self) -> CircuitState:
        ...
        try:
            ...
            if self._fallback_on_error:
                logger.error(
                    "DistributedCircuitBreaker.state read failed: %s", _redact_exc(exc)
                )
                return self._fallback.state
            raise
    ...
    def failure_count(self) -> int:
        ...
        try:
            ...
            if self._fallback_on_error:
                logger.error(
                    "DistributedCircuitBreaker.failure_count read failed: %s",
                    _redact_exc(exc),
                )
                return self._fallback.failure_count
            raise
    ...
    def success_count(self) -> int:
        ...
        try:
            ...
            if self._fallback_on_error:
                logger.error(
                    "DistributedCircuitBreaker.success_count read failed: %s",
                    _redact_exc(exc),
                )
                return self._fallback.success_count
            raise
    ...
    def reset(self) -> None:
        ...
        try:
            ...
            if self._fallback_on_error:
                logger.error(
                    "DistributedCircuitBreaker.reset failed: %s", _redact_exc(exc)
                )
                self._fallback.reset()
            else:
                raise
