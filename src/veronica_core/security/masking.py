...
_PATTERNS: list[tuple[str, re.Pattern[str]]] = [
    ...
    (
        "STRIPE_KEY",
        re.compile(r"\b(sk_live_[A-Za-z0-9]{24,}|pk_live_[A-Za-z0-9]{24,})\b"),
    ),
    # Firebase / Supabase service role JWTs (long JWT tokens with known header)
    (
        "SERVICE_ROLE_JWT",
        re.compile(
            r"\beyJ[A-Za-z0-9+/=]{40,}\.[A-Za-z0-9+/=]{40,}\.[A-Za-z0-9\-_]{20,}\b"
        ),
    ),
    # Resend API keys
    ...
    (
        "TWILIO_TOKEN",
        re.compile(
            r"(?i)twilio[_\-. ]?(?:auth[_\-. ]?)?token\s*[=:]\s*([^\s,;\"']{20,})"
        ),
    ),
    # SendGrid API keys
    ...
    (
        "AZURE_SAS",
        re.compile(r"(?i)(?:sig|SharedAccessSignature)[=&]([A-Za-z0-9%+/=]{20,})"),
    ),
    ...
    (
        "POLYMARKET_KEY",
        re.compile(
            r"(?i)polymarket[_\-. ]?(?:api[_\-. ]?)?(?:key|secret|token)\s*[=:]\s*([^\s,;\"']{20,})"
        ),
    ),
    ...
    def _mask_value(self, key: str, value: Any, _depth: int = 0) -> Any:
        ...
        if _depth >= self._MAX_DEPTH:
            ...
            if isinstance(value, dict):
                return {
                    k: self.mask(v)
                    if isinstance(v, str)
                    else self.mask(v.decode("utf-8", errors="replace"))
            ...
            if isinstance(value, (list, tuple)):
                masked = [
                    ...
                    else self.mask(item.decode("utf-8", errors="replace"))
        ...
        if isinstance(value, bytes):
            return self._mask_value(
                key, value.decode("utf-8", errors="replace"), _depth
            )
        if isinstance(value, str):
            return self.mask(value)
        if isinstance(value, dict):
